...
class DataSet():
    ...
    def __init__(self, seq_length=20, class_limit=None, image_shape=(200, 200, 3)):
        ...
        # Get the classes.
        self.classes = 3     #self.get_classes()
    ...
    def clean_data(self):
        ...
        for item in self.data:
            if int(item[6]) >= self.seq_length and int(item[6]) <= self.max_frames and item[1] in self.classes:
    ...
    def get_all_sequences_in_memory(self, train_val, data_type):
        ...
        for row in data:
            ...
            # print(row)
            X.append(sequence)
            y.append([float(row[2]), float(row[3]), float(row[3])])
            # import pdb; pdb.set_trace()
    ...
    def frame_generator(self, batch_size, train_val, data_type):
        ...
        while 1:
            ...
            for _ in range(batch_size):
                ...
                X.append(sequence)
                y.append([float(sample[2]), float(sample[3]), float(sample[4])])
    ...
    def get_extracted_sequence(self, data_type, sample):
        ...
        filename = str('{:04d}'.format(int(sample[1])))
    ...
    def get_frames_for_sample(sample):
        ...
        filenames."""
        #import pdb; pdb.set_trace()
        path = 'Data/' + sample[0] + '/' + str('{:04d}'.format(int(sample[1]))) + '/'
        filename = sample[5]
